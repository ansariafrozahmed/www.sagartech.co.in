...
if(isset($_POST['cl_submit'])) {
    ...
    $cl_pass = $_POST['cl_password'];
    // $act_cont = $_POST['cl_interest'];

    $act = $conn->prepare("INSERT INTO guest (cl_username,cl_password) VALUES(?,?)");
    if ($act->execute([$cl_username,$cl_pass])) { 
...
else if(isset($_POST['cl_update'])) {
        
        $cl_id       = $_POST['cl_id']; 
        $cl_username = $_POST['cl_username'];
        $cl_pass     = $_POST['cl_password'];
        // $cl_inter    = $_POST['cl_interest'];
        
        $act = $conn->prepare("UPDATE guest SET cl_username = ?,cl_password = ? WHERE cl_id = ? ");
    if ($act->execute([$cl_username,$cl_pass,$cl_id])) { 
   // it worked
                  ...
                  <h6 class="bigstats"><?php //echo $statusMsg;?></h6>
                    ...
                    <fieldset>		
                      <div class="control-group">											
                        <label class="control-label" for="cl_name">Client Username</label>
                        <div class="controls">
                          <input type="text" class="span4" id="cl_name" name="cl_username" placeholder="Client Username" required>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="control-group">											
                        <label class="control-label" for="cl_password">Client Password</label>
                        <div class="controls">
                          <input type="text" class="span4" id="cl_password" name="cl_password" placeholder="Client Password" required>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="form-actions">
											<button type="submit" name="cl_submit" class="btn btn-primary">Add</button>
										</div> <!-- /form-actions -->
                    </fieldset>
                  </form>
                </div>
          ...
          <div class="widget">
            ...
            <table>
              <thead>
                ...
                ?>
				<tr class="act_tr">
					<td class="nr" hidden><?php echo $act['cl_id'];?></td>
          <td><?php echo $i;?></td>
					<td><?php echo $act['cl_username'];?></td>
        ...
        ?>
         ...
         <div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                                      <div class="modal-header">
                                                        ...
                                                        <h3 id="myModalLabel">Wants to delete this Client's Credentials?</h3>
                                                      </div>
                                                      <div class="modal-body">
                                                        <p>This Detail will be deleted and can't be undo.</p>
                                                      </div>
...
$(document).ready(function() {
      ...
      $('td #actDel').click(function() {
          ...
          $('#modalYes').click(function(){
             window.location.href = "include/del_data.php?cl_id="+id;
          });
      });    
