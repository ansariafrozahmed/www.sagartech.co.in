...
if(isset($_POST['cat_submit'])) {
    ...
    $act = $conn->prepare("INSERT INTO category (cat_name) VALUES(?)");
    $act->execute([$cat_name]);
}
else if(isset($_POST['thm_submit'])) {
    $thm_name = $_POST['thm_name'];
    $thm_link = $_POST['thm_link'];
    $thm_cat = $_POST['thm_cat'];
    
     $filename = $_FILES['files']['name'];

    // Location
    $target_file = '../images/themes/'.$filename;
    ...
    $act = $conn->prepare("INSERT INTO theme (thm_cat_id,thm_name,thm_link,thm_img) VALUES(?,?,?,?)");
    $act->execute([$thm_cat,$thm_name,$thm_link,$filename]);
...
else if(isset($_POST['thm_update'])) {
        
        $act_name = $_POST['thm_name'];
        $act_date = $_POST['thm_link'];
        $last_id = $_POST['thm_id'];
        $filename = $_FILES['files']['name'];
if($filename == '') {
        $sql = mysqli_query($con,"SELECT thm_img FROM theme WHERE thm_id = '$last_id'");
        $row = mysqli_fetch_assoc($sql);
        $filename = $row['thm_img'];
    }
    // Location
    $target_file = '../images/themes/'.$filename;
    ...
    if(in_array($file_extension, $valid_extension)){
       ...
       move_uploaded_file($_FILES['files']['tmp_name'],$target_file);
        ...
        $act = $conn->prepare("UPDATE theme SET thm_name = ?,thm_link = ?,thm_img = ? WHERE thm_id = ? ");
...
.wraps { 
    width: 370px; height: 240px; padding: 0; overflow: hidden; float: left;
                  ...
                  <h6 class="bigstats"><?php //echo $statusMsg;?>Upload Images with normal resolution so that in home page noting go wrong</h6>
                    ...
                    <fieldset>	
                      <div class="control-group">											
                        <label class="control-label" for="act_name">Select Catergory</label>
                        <div class="controls">
                          <select class="span2" id="thm_cat" name="thm_cat">
                      ...
                      <div class="control-group">											
                        ...
                        <div class="controls">
                          <input type="text" class="span4" id="act_name" name="thm_name" placeholder="Theme Name" required>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="control-group">											
                        <label class="control-label" for="date">Theme Link</label>
                        <div class="controls">
                        <input type="text" class="span4" name="thm_link" placeholder="Theme link" autocomplete="off" required>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="control-group">											
                        <label class="control-label" for="act_name">Theme Image</label>
                        <div class="controls">
                          <input type="file" class="span4" id="act_img" name="files"  multiple>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="form-actions">
											<button type="submit" name="thm_submit" class="btn btn-primary">Add</button>
										</div> <!-- /form-actions -->
                    </fieldset>
                  </form>
                </div>
        ...
        <div class="span5">
          <div class="widget widget-nopad">
            ...
            <div class="widget-content">
              <div class="widget big-stats-container">
                <div class="widget-content">
                  <h6 class="bigstats"><?php //echo $statusMsg;?>Add Category</h6>
                    <form id="edit-profile" class="form-horizontal" method="POST" action="" enctype="multipart/form-data">
                    <fieldset>
                      <div class="control-group">											
                        <label class="control-label" for="act_name">Category Name</label>
                        <div class="controls">
                          <input type="text" class="span3" id="Category_name" name="cat_name" placeholder="Category Name" required>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="form-actions">
        ...
        <div class="span6">
          <div class="widget">
            <div class="widget-header"> <i class="icon-picture"></i>
              <h3>Themes </h3> 
              <!--<h5>Drag and drop to change position</h5>-->
            </div>
            <!-- /widget-header -->
            <div class="widget-content">
                    <div class="control-group">											
                    <label class="control-label" for="act_name">Select Themes by Catergory</label>
                        <div class="controls">
                          <select class="span2" id="thmShow" name="grap_name">
                              <option value="null">-SELECT-</option>
                              <?php 
                                $sql = mysqli_query($con,"SELECT * FROM category");
                                while($rows = mysqli_fetch_assoc($sql)) {
         ...
         <div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                                      <div class="modal-header">
                                                        ...
                                                        <h3 id="myModalLabel">Wants to delete this theme?</h3>
                                                      </div>
                                                      <div class="modal-body">
                                                        <p>This theme will be deleted and can't be undo.</p>
                                                      </div>
...
$(document).ready(function() {
    ...
    $('td #thmDel').click(function() {
          var id = $(this).parent().parent().find(".nr").text();
          $('#modalYes').click(function(){
             window.location.href = "include/del_data.php?thm_id="+id;
          });
      });
