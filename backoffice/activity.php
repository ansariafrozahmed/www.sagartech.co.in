...
if(isset($_POST['img_submit'])) {
    ...
    $act_cont = $_POST['act_content'];
    
    $act = $conn->prepare("INSERT INTO activity (act_name,act_date,act_content) VALUES(?,?,?)");
    if ($act->execute([$act_name,$act_date,$act_cont])) { 
   // it worked
    // Count total files
  ...
  // Prepared statement
  $query = "INSERT INTO images (img_act_id,img_name,img_tag) VALUES(?,?,?)";
  ...
  for($i=0;$i<$countfiles;$i++){
    ...
    if(in_array($file_extension, $valid_extension)){
       ...
       if(move_uploaded_file($_FILES['files']['tmp_name'][$i],$target_file)){
          ...
          // Execute query
	  $statement->execute(array($last_id,$filename,$target_file));
  ...
  echo "File upload successfully";
        ...
        $act_date = $_POST['act_date'];
        $act_cont = $_POST['act_content'];
        $last_id = $_POST['act_id'];
        $act = $conn->prepare("UPDATE activity SET act_name = ?,act_date = ?,act_content = ? WHERE act_id = ? ");
    if ($act->execute([$act_name,$act_date,$act_cont,$last_id])) { 
   // it worked
    // Count total files
    //$last_id = $conn->lastInsertId();
  $countfiles = count($_FILES['files']['name']);
...
else if(isset($_POST['act_submit'])) {
    $graphic = $_POST['act_name'];

  $countfiles = count($_FILES['files']['name']);
 if($graphic == 'achieve') {
    $query = "INSERT INTO images (img_act_id,img_tag) VALUES(5,?)";
 }
 else if($graphic == 'intern') {
    $query = "INSERT INTO images (img_act_id,img_tag) VALUES(6,?)";
 }
 else {
    $error = "Please select graphics work";
}
  // Prepared statement
  ...
  for($i=0;$i<$countfiles;$i++){
    ...
    $filename = $_FILES['files']['name'][$i];
if($graphic == 'achieve') {
    $imgloc = '../images/achievements/'.$filename;
 }
 else if($graphic == 'intern') {
    $imgloc = '../images/interns/'.$filename;
 }
...
else if(isset($_POST['act_update'])) {
        
        $act_id   = $_POST['img_id'];
        $img_act_id = $_POST['img_act_id'];
        $act_name = $_POST['act_name'];
        $filename = $_FILES['files']['name'];
 if($$img_act_id == '1') {
     $target_file = '../images/logo/'.$filename;
 }
 else if($graphic == 'poster') {
     $target_file = '../images/posters/'.$filename;
 }
 else if($graphic == 'pamplet') {
     $target_file = '../images/pamplets/'.$filename;
 }
 else {
    $error = "Please select graphics work";
}
    ...
    if(in_array($file_extension, $valid_extension)){
       ...
       if(move_uploaded_file($_FILES['files']['tmp_name'],$target_file)){
        $query = "UPDATE images SET img_name = ? WHERE img_id = ? ";
        $act = $conn->prepare();
        $act->execute([$act_name,$act_date,$filename,$last_id]);
  echo "File upload successfully";
    }
    else {
        echo "Something went wrong";
...
<head>
<meta charset="utf-8">
<title>Activity Gallery - Backoffice</title>
      ...
      <div class="row">
        <div class="span6">
          <div class="widget widget-nopad">
            <div class="widget-header"> <i class="icon-list-alt"></i>
              <h3>Activity Upload</h3>
            </div>
            <!-- /widget-header -->
            <div class="widget-content">
              <div class="widget big-stats-container">
                <div class="widget-content">
                  <h6 class="bigstats"><?php //echo $statusMsg;?>Upload Images with normal resolution so that in home page noting go wrong</h6>
                    ...
                    <fieldset>	
                      <div class="control-group">											
                        ...
                        <div class="controls">
                          <input type="text" class="span4" id="act_name" name="act_name" placeholder="Activity Name" required>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="control-group">											
                        <label class="control-label" for="date">Activity Date</label>
                        <div class="controls">
                        <input type="text" class="form-control docs-date" class="span4" name="act_date" placeholder="Pick a date" autocomplete="off" required>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="control-group">											
                        <label class="control-label" for="date">Activity Content</label>
                        <div class="controls">
                        <textarea class="span4" name="act_content"></textarea>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="control-group">											
                      ...
                      <div class="form-actions">
											<button type="submit" name="img_submit" class="btn btn-primary">Add</button>
										</div> <!-- /form-actions -->
                    </fieldset>
                  </form>
                </div>
          ...
          <div class="widget widget-nopad">
            ...
            <!-- /widget-header -->
                <div class="widget-content">
                  <div class="widget big-stats-container">
                    <div class="widget-content">
                      <h6 class="bigstats"><?php //echo $statusMsg;?>Image upload section : upload images as per the guide line given by developer</h6>
                        <form id="edit-profile" class="form-horizontal" method="POST" action="" enctype="multipart/form-data">
                        <fieldset>	
                          <div class="control-group">											
                            <label class="control-label" for="graphic">Select Activity:</label>
                            <div class="controls">
                              <select class="span2" id="graphic" name="act_name">
                          ...
                          <div class="control-group">											
                            <label class="control-label" for="act_img">Images</label>
                            <div class="controls">
                              <input type="file" class="span3" id="act_img" name="files[]"  multiple>
                            </div> <!-- /controls -->				
                          </div> <!-- /control-group -->
                          <div class="form-actions">
    											<button type="submit" name="act_submit" class="btn btn-primary">Add</button>
    										</div> <!-- /form-actions -->
                        </fieldset>
                      </form>
                    </div>
        ...
        <div class="span6">
          <div class="widget">
            ...
            <div class="widget-content">
                <div class="control-group">											
                    <label class="control-label" for="act_name">Select Activity</label>
                        <div class="controls">
                          <select class="span2" id="grapShow" name="grap_name">
                              <option value="null">-SELECT-</option>
                              <option value="coreact">Activities</option>
                              <option value="achieve">Achievements</option>
                              <option value="intern">Interns</option>
                          </select>
                        </div> <!-- /controls -->				
            ...
            <table class="coreAct">
              <thead>
                ...
                ?>
				<tr class="act_tr">
					<td class="nr" hidden><?php echo $act['act_id'];?></td>
          <td><?php echo $i;?></td>
        ...
        ?>
         ...
         <div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                                      <div class="modal-header">
                                                        ...
                                                        <h3 id="myModalLabel">Wants to delete this Activity?</h3>
                                                      </div>
                                                      <div class="modal-body">
                                                        <p>This activity will be deleted and can't be undo.</p>
                                                      </div>
...
$(document).ready(function() {
      ...
      $('td #actDel').click(function() {
          ...
          $('#modalYes').click(function(){
             window.location.href = "include/del_data.php?act_id="+id;
          });
      });    
