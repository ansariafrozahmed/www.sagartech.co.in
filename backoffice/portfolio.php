...
if(isset($_POST['port_submit'])) {
    $act_name = $_POST['port_name'];
    $act_date = $_POST['port_link'];
    ...
    $act = $conn->prepare("INSERT INTO portfolio (port_name,port_link,port_img) VALUES(?,?,?)");
    $act->execute([$act_name,$act_date,$filename]);
...
else if(isset($_POST['port_update'])) {
        
        $act_name = $_POST['port_name'];
        $act_date = $_POST['port_link'];
        $last_id = $_POST['port_id'];
        $filename = $_FILES['files']['name'];
    if($filename == '') {
        $sql = mysqli_query($con,"SELECT port_img FROM portfolio WHERE port_id = '$last_id'");
        $row = mysqli_fetch_assoc($sql);
        $filename = $row['port_img'];
    }
    ...
    //   {
        ...
        $act = $conn->prepare("UPDATE portfolio SET port_name = ?,port_link = ?,port_img = ? WHERE port_id = ? ");
                  ...
                  <h6 class="bigstats"><?php //echo $statusMsg;?>Upload Images with normal resolution so that in home page noting go wrong</h6>
                    ...
                    <fieldset>	
                      <div class="control-group">											
                        <label class="control-label" for="act_name">Project Name</label>
                        <div class="controls">
                          <input type="text" class="span6" id="act_name" name="port_name" placeholder="Activity Name" required>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="control-group">											
                        <label class="control-label" for="date">Project Link</label>
                        <div class="controls">
                        <input type="text" class="span6" name="port_link" placeholder="Project link" autocomplete="off" required>
                        </div> <!-- /controls -->				
                      </div> <!-- /control-group -->
                      <div class="control-group">											
                      ...
                      <div class="form-actions">
											<button type="submit" name="port_submit" class="btn btn-primary">Add</button>
										</div> <!-- /form-actions -->
                    </fieldset>
                  </form>
                </div>
        ...
        <div class="span6">
          <div class="widget">
            <div class="widget-header"> <i class="icon-picture"></i>
              <h3>Portfolios </h3> <h5>Drag and drop to change position</h5>
            </div>
            <!-- /widget-header -->
            <div class="widget-content">
            <table>
              <thead>
				...
				<?php
                $i = 1;
                $stmt = $conn->prepare("SELECT * FROM portfolio ORDER BY port_pos");
                $stmt->execute();
                $acts = $stmt->fetchAll();
                foreach ($acts as $act) { 
                    $pos = $act['port_id'];
                ?>
				<tr class="act_tr" id="<?php echo $pos; ?>">
					<td class="nr" hidden><?php echo $act['port_id'];?></td>
					<td id="posTd"><?php echo $i;?></td>
					<td><?php echo $act['port_name'];?></td>
					<td><a href="#myModal" role="button" class="btn btn-danger portDel" id="portDel" data-toggle="modal">Delete</a></td>
				</tr>
        <?php
        $i++;
                }
...
$(document).ready(function() {
    ...
    $('td #portDel').click(function() {
          var id = $(this).parent().parent().find(".nr").text();
          $('#modalYes').click(function(){
             window.location.href = "include/del_data.php?port_id="+id;
          });
      });
