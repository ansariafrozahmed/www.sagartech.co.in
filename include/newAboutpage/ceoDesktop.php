<?php include('include/config.php') ?>
<div class="lg:px-20 px-4 md:pt-16  py-10 max-sm:pb-0 bg-gray-100 ">

    <div class=" flex justify-between lg:flex-row flex-col max-sm:items-center gap-10 relative  bg-gray-100  ">

        <div class=" lg:w-[35%] w-full lg:sticky top-24 h-full ">
            <div class="max-sm:flex justify-center" data-aos="fade-up">

                <span data-aos="fade-up" class="inline-block  bg-gray-100 px-4 py-2 rounded-full text-xs font-light text-[#242424]">CEO's</span>
            </div>
            <h2 class="text-[28px]  lg:text-[40px] max-sm:text-center mt-4 leading-[1.15] font-[400] text-[#242424] heading-all" data-aos="fade-up">
                Insights and Reflections from the Desk of Our CEO

            </h2>
            <div class="space-y-4 lg:mt-12 mt-7">
                <p class="content-all text-[13.5px] font-bold tracking-wide text-gray-600 " data-aos="fade-up">
                    <span class="text-red-600 ">"</span> if we do not give our clients the desired results they expect and for which they have approached us, then our income will not be appropriate.<span class="text-red-600 font-bold"> "</span>
                </p>
                <hr>
                <div class="">
                    <div class="flex items-center justify-between space-x-3">
                        <div data-aos="fade-up">
                            <p class="mb-1 text-sm content-all text-gray-600">Written by,</p>
                            <p class="text-red-500 flex gap-1 items-center text-sm heading-all">
                                Ubaid Saudagar - <span class="text-gray-500 ">CEO, Sagar Tech</span>
                            </p>
                        </div>
                        <img src="./images/CEO.webp" alt="" class="w-12 h-12 rounded-full bg-gray-200" data-aos="fade-up" />
                    </div>
                </div>
            </div>

        </div>

        <div class=" lg:w-[65%] w-full">
            <div class="rounded-2xl lg:h-[400px] overflow-hidden" data-aos="fade-up">

                <img src="ceodesk2.webp" alt="" class=" ">
            </div>
            <div class="flex lg:flex-row flex-col max-sm:items-center justify-between gap-5 mt-8">
                <div class="md:w-1/2 w-full text-justify space-y-3">
                    <p class="text-[13.5px] font-light tracking-wide text-gray-500" data-aos="fade-up">
                        Our basic principle, which we have been successful in the past few years, is TRUST. We have not broken the trust of our clients in terms of the development of websites, <a href="/social-media-optimization-agency-in-mumbai" class="text-red-600">social media optimization, SEO</a>, or any other service for which our client has opted for our services. Hence, our client retention percentage is 90%.
                    </p>
                    <p class="text-[13.5px] font- tracking-wide text-gray-500" data-aos="fade-up">
                        When clients get the desired results they want, then automatically supplications come from their hearts, and they give us a positive review and feedback.
                    </p>


                </div>
                <div class="md:w-1/2 w-full text-justify space-y-3">

                    <p class="text-[13.5px] font-light tracking-wide text-gray-500" data-aos="fade-up">
                        In terms of development, we continuously keep upgrading with the latest technology. So, when we started <a href="/web-development-company-in-mumbai" class="text-red-600">Website Development</a> in 2018, we just had one technology to develop our websites, and that was WordPress, along with a shared server. Then, gradually, we moved towards <a href="/bootstrap-development-services-mumbai" class="">Bootstrap</a>, PHP for building custom websites. Now, we have adapted ourselves to the latest trends for developing our websites, wherein we use Remix, <a href="/nextjs-development-services-in-india" class="text-red-600">Next Js</a>, <a href="/nodejs-development-company-in-india" class="text-red-600">Node Js</a>, JWT for security, and many more.
                    </p>

                </div>
            </div>
        </div>

    </div>
    <div>

    </div>
</div>